...
import { splitAndCheckPath } from "../interpreter/path"
...
function staticPath(node: RawNode | undefined, at: RawNode): string {
  ...
  // Validate segments via the shared checker (same rules as run-action) so a bad
  // path is an author-time error, not a render-time crash.
  const r = splitAndCheckPath(node.value)
  if ("error" in r) throw new CompileError(r.error, locOf(node))
  return node.value
}
