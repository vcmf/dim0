...
- "Compare France's and Germany's economies." → an applet table or chart — the comparison *is* the answer; don't wait to be asked for one.
...
- `create_folder(label)`: make a new folder (a nested sub-board) in the current working folder; returns its id. To build a self-contained sub-board, `create_folder` → `navigate(folder_id)` → author the notes/links inside → `navigate("up")` back out. Use this when the user asks for a structured collection that deserves its own space (a project, a topic, a study set) rather than scattering notes on the current board.
